...
function buildPriorities(checks: Checks, housing: string, insVal: number) {
  ...
  if (checks.kabu) cards.push({ num:n(idx++), highlight:false, title:"持株会の奨励金を最大活用しましょう", desc:"持株会の奨励金は、使えるなら最大限活用したい制度です。まずは奨励金が出る上限の掛金額を確認し、無理のない範囲で活用するのがお勧めです。一方で、自社株に偏りすぎるとリスクも高まるため、その点は注意しましょう。", cta:"LINEで詳しく相談する", style:"green", href:"line" });
  ...
  if (checks.hoken && housing==="持ち家（ローンあり）" && checks.loan3) cards.push({ num:"確認事項", highlight:false, title:"生命保険料を見直せる可能性があります", desc:"住宅ローンに3大疾病特約付き団信をつけている場合、保障が重複している可能性があります。", note:"※ご自身の保険内容を確認した上で専門家にご相談ください。", cta:"LINEで保険を一緒に確認する", style:"green", href:"line" });
  if (checks.hoken && insVal > 0) cards.push({ num:"確認事項", highlight:false, title:"変額保険・積立保険の見直しを検討しましょう", desc:"保険会社の変額保険で運用するよりもNISAの方が税制上得です。保障が必要なら変額保険ではなく、定額保険で十分です。運用は運用、保障は保険の方が一般的にコストが下がります。", note:"", cta:"LINEで保険を一緒に見直す", style:"green", href:"line" });
